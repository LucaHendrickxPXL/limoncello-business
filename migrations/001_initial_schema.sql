...
create table if not exists document_sequences (
  ...
  primary key (scope, sequence_key),
  constraint document_sequences_last_value_check
    check (last_value >= 0)
);
...
drop view if exists order_metrics_v1;
drop view if exists batch_metrics_v1;
drop view if exists article_reporting_v1;

create view batch_metrics_v1 as
select
  b.id as batch_id,
  b.batch_number,
  b.status,
  ...
  coalesce(ordered.ordered_liters, 0)::numeric(12, 3) as ordered_liters,
  ...
  (
    ...
    - coalesce(ordered.ordered_liters, 0)
    - coalesce(sold.sold_liters, 0)
  )::numeric(12, 3) as bookable_liters,
  (
...
left join (
  select
    o.batch_id,
    sum(o.ordered_liters) as ordered_liters
  from orders o
  where o.status in ('besteld', 'in_verwerking', 'klaar_voor_uitlevering')
  group by o.batch_id
) ordered on ordered.batch_id = b.id
left join (
  select
    o.batch_id,
...
create view order_metrics_v1 as
with batch_source as (
  select
    ...
    coalesce(costs.cost_amount, 0)::numeric(14, 2) as batch_cost_amount
  from batches b
  left join (
    select
      e.batch_id,
...
select
  o.id as order_id,
  coalesce(
    round((o.ordered_liters * (bs.batch_cost_amount / nullif(bs.source_liters, 0)))::numeric, 2),
    0
  )::numeric(14, 2) as cost_amount,
  (
    o.total_amount
    - coalesce(
      round((o.ordered_liters * (bs.batch_cost_amount / nullif(bs.source_liters, 0)))::numeric, 2),
  ...
  (o.status in ('in_verwerking', 'klaar_voor_uitlevering')) as reserves_batch_capacity
from orders o
join batch_source bs on bs.batch_id = o.batch_id;

create view article_reporting_v1 as
select
  a.id as article_id,
  a.name,
  a.category,
